...
for i in range(0,len(directory_contents)):
    ...
    for filename in glob.iglob(newpath + '**/*.jpg', recursive=True):
        ...
        with image as im:
            ...
            im_crop = im #im.crop((left, upper, right, lower))
...
#p#rint(np.asarray(y_train).shape())
...
fig, ax = plt.subplots()
# title for the plots
title = ('SVM: Demonstration with N=2 ')
...
# print(np.shape(X))
# a_list = list(range(0, nrows))
#
# #plotting stuff here
# plot1=plt.figure(2)
#
# plt.scatter(a_list, X[:,5875], s=30, cmap=plt.cm.Paired)
#
#
#
...
#
# getlimits = plt.gca()
# xmax = getlimits.get_xlim()
# ymax = getlimits.get_ylim()
#
# print(xmax)
# print(ymax)
#
# xgrid = np.linspace(-xmax[0], xmax[1], 30)
# ygrid = np.linspace(-ymax[0], ymax[1], 32)
# ymeshgrid, xmeshgrid = np.meshgrid(ygrid, xgrid)
# xy = np.vstack([xmeshgrid.ravel(), ymeshgrid.ravel()]).T
# Z = clf.decision_function(xy).reshape(ymeshgrid.shape)
#
# getlimits.contour(xmeshgrid, ymeshgrid, Z, colors='k', levels=[-1, 0, 1], alpha=0.5,
#            linestyles=['--', '-', '--'])
#
#
# getlimits.scatter(clf.support_vectors_[:, 0], clf.support_vectors_[:, 1], s=100,
